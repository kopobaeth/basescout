...
export default function StocksCanvas() {
  ...
  useEffect(() => {
    ...
    const wheel = (e: WheelEvent) => { if (blocked.current) return; e.preventDefault(); setTouched(true); zoom(e.clientX, e.clientY, Math.exp(-e.deltaY * (e.deltaMode === 1 ? .035 : .0015))); };
    ...
    frame = requestAnimationFrame(draw);
    return () => { cancelAnimationFrame(frame); el.removeEventListener("pointerdown", down); el.removeEventListener("pointermove", move); el.removeEventListener("pointerup", up); el.removeEventListener("pointercancel", up); el.removeEventListener("wheel", wheel); el.removeEventListener("keydown", keys); };
  }, []);
